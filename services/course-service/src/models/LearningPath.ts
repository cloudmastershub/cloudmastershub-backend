...
LearningPathSchema.pre('validate', async function(next) {
  try {
    ...
    if (!this.slug && this.title) {
      console.log('🔧 Generating slug before validation for learning path:', this.title);
      ...
      this.slug = baseSlug;
      console.log('🔧 Set initial slug for validation:', this.slug);
    }
    
    next();
  } catch (error: any) {
    console.error('🔧 Error in LearningPath pre-validation middleware:', error);
    ...
    if (!this.slug) {
      ...
      console.log('🔧 Emergency slug set in validation error handler:', this.slug);
    }
...
LearningPathSchema.pre('save', async function(next) {
  try {
    ...
    if (this.isNew || this.isModified('title')) {
      console.log('🔧 Ensuring slug uniqueness for learning path:', this.title);
      ...
      try {
        ...
        console.log('🔧 Fallback unique slug:', this.slug);
      }
    }
    ...
    console.log('🔧 Pre-save middleware completed successfully for learning path');
    next();
  } catch (error: any) {
    console.error('🔧 Error in LearningPath pre-save middleware:', error);
