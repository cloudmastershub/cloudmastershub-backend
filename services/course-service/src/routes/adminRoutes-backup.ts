...
/**
 * Fix course category validation issues
 * Convert uppercase category values to lowercase to match enum schema
 */
router.post('/courses/fix-categories', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    logger.info('Admin requesting course category fixes', { adminId: req.userId });

    // Find courses with uppercase categories
    const coursesWithUppercaseCategories = await Course.find({
      category: { $in: ['AWS', 'AZURE', 'GCP', 'MULTICLOUD', 'DEVOPS', 'SECURITY', 'DATA'] }
    });

    if (coursesWithUppercaseCategories.length === 0) {
      res.json({
        success: true,
        data: {
          fixedCourses: [],
        ...
        message: 'No courses need category fixes'
      });
      return;
    }

    logger.info(`Found ${coursesWithUppercaseCategories.length} courses with uppercase categories:`, {
      courses: coursesWithUppercaseCategories.map(c => ({ 
        id: c._id, 
        title: c.title, 
    ...
    const fixedCourses: Array<{id: string, title: string, oldCategory: string, newCategory: string}> = [];
    let totalFixed = 0;

    // Update each course individually for better error handling
    for (const course of coursesWithUppercaseCategories) {
      const oldCategory = course.category;
      const newCategory = categoryMapping[oldCategory as string];
      
      if (newCategory) {
        try {
          // Set the category directly without going through validation
          ...
          logger.info(`Fixed course category: ${course.title}`, {
            courseId: course._id,
            oldCategory,
            newCategory
          });
        } catch (error: any) {
          logger.error(`Failed to fix category for course: ${course.title}`, {
            courseId: course._id,
            error: error.message
          });
        }
    ...
    logger.info(`Successfully fixed ${totalFixed} course categories`, {
      adminId: req.userId,
      totalFixed,
      fixedCourses
    });
    ...
    res.json({
      ...
      message: `Successfully fixed ${totalFixed} course categories`
    });

  } catch (error: any) {
    logger.error('Failed to fix course categories:', error);
    res.status(500).json({
      success: false,
      error: {
        message: 'Failed to fix course categories',
        details: error.message
      }
    });
  }
